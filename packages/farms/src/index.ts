...
export function createFarmFetcher(multicallv2: MultiCallV2) {
  const fetchFarms = async (
    ...
    const masterChefAddress = isTestnet ? masterChefAddresses[ChainId.ZETA_TESTNET] : masterChefAddresses[ChainId.CMP]
  ...
  return {
    ...
    supportedChainId,
    isTestnet: (chainId: number) => ![ChainId.CMP, ChainId.ETHEREUM].includes(chainId),
  }
}
